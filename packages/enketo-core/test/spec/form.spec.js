...
describe('Output functionality ', () => {
    ...
    it('tested upon initialization: node random__', () => {
        const val = Number(
            form.view.html.querySelector('[data-value="/random/random__"]')
                .textContent
        );
        expect(val)
            .to.be.a('number')
            .and.to.be.greaterThanOrEqual(0)
            .and.to.be.lessThanOrEqual(1);
    });

    it('tested upon initialization: node uuid__', () => {
        const val = form.view.html.querySelector(
            '[data-value="/random/uuid__"]'
        ).textContent;
...
describe('Preload and MetaData functionality', () => {
    ...
    it('ignores a calculate binding on /[ROOT]/meta/instanceID', () => {
        ...
        form.init();
        expect(
            form.model.node('/random/meta/instanceID').getVal().length
        ).to.equal(41);
    });
    ...
    it('ignores a calculate binding on [ROOT]/orx:meta/orx:instanceID', () => {
        ...
        form.init();
        expect(
            form.model.node('/data/orx:meta/orx:instanceID').getVal().length
        ).to.equal(41);
    });
    ...
    it('generates an instanceID on /[ROOT]/meta/instanceID WITHOUT preload binding', () => {
        ...
        expect(form.view.$.find('fieldset#or-preload-items').length).to.equal(
            0
        );
        expect(
            form.model.node('/random/meta/instanceID').getVal().length
    ...
    it('generates an instanceID WITH a preload binding', () => {
        ...
        form.init();
        const sel =
            'fieldset#or-preload-items input[name="/preload/meta/instanceID"][data-preload="uid"]';
        expect(form.view.$.find(sel).length).to.equal(1);
        expect(
            form.model.node('/preload/meta/instanceID').getVal().length
        ).to.equal(41);
    });
    ...
    it('does not generate a new instanceID if one is already present', () => {
        const form = new Form(
            document
                ...
                .createContextualFragment(
                    `<div>${forms['random.xml'].html_form}</div>`
                )
                .querySelector('form'),
            {
                modelStr: forms['random.xml'].xml_model.replace(
                    '<instanceID/>',
        ...
        form.init();
        expect(form.model.node('/random/meta/instanceID').getVal()).to.equal(
            'existing'
        );
    });
    ...
    it('generates a timeStart on /[ROOT]/meta/timeStart WITH a preload binding', () => {
        const form = loadForm('preload.xml');
        form.init();
        expect(form.model.node('/preload/start').getVal().length > 10).to.equal(
            true
        );
    });
    ...
    it('also works with nodes that have a corresponding form control element', () => {
        ...
        [
            '/dynamic-default/two',
            '/dynamic-default/four',
            '/dynamic-default/six',
        ].forEach((path) => {
            expect(
                form.view.$.find(`[name="${path}"]`).val().length > 9
            ).to.equal(true);
    ...
    it('some session context can be passed to the data.session property when instantiating form', () => {
        ...
        [
            ...
            expect(form.model.node(`/preload/${prop}`).getVal()).to.equal(
                session[prop]
            );
        });
    });
    ...
    function testPreloadExistingValue(node) {
        it(`obtains unchanged preload value of item (WITH preload binding): ${node.selector}`, () => {
            ...
            form.init();
            expect(form.model.node(node.selector).getVal()).to.equal(
                node.result
            );
        });
    }
    ...
    function testPreloadNonExistingValue(node) {
        it(`populates previously empty preload item (WITH preload binding): ${node.selector}`, () => {
            const form = loadForm('preload.xml');
            form.init();
            expect(form.model.node(node.selector).getVal().length > 0).to.equal(
                true
            );
        });
    }
...
describe('Loading instance values into html input fields functionality', () => {
    ...
    it('correctly populates input fields of non-repeat node names in the instance', () => {
        ...
        expect(
            form.view.$.find('[name="/thedata/repeatGroup/nodeC"]').eq(2).val()
        ).to.equal('c3');
        expect(form.view.$.find('[name="/thedata/nodeX"]').val()).to.equal(
            undefined
        );
    });
    ...
    it('correctly populates if the first radiobutton or first checkbox only has a value', () => {
        ...
        form.init();
        const $input = form.view.$.find('[data-name="/issue208/rep/nodeA"]').eq(
            0
        );
...
describe('calculations', () => {
    ...
    it('also work inside repeats', () => {
        ...
        form.init();
        form.view.$.find('.add-repeat-btn').click();
        form.view.$.find('[name="/calcs_in_repeats/rep1/num1"]:eq(0)')
            .val('10')
            .trigger('change');
        form.view.$.find('[name="/calcs_in_repeats/rep1/num1"]:eq(1)')
            .val('20')
            .trigger('change');
        expect(
            form.model.node('/calcs_in_repeats/rep1/grp/num1', 0).getVal()
        ).to.equal('200');
        expect(
            form.model.node('/calcs_in_repeats/rep1/grp/num1', 1).getVal()
        ).to.equal('400');
    });
    ...
    it('are not performed if the calculation is not relevant', () => {
        ...
        form.view.$.find('[name="/calcs_in_repeats/rep1/num1"]:eq(1)')
            .val('5')
            .trigger('change');
        form.view.$.find('[name="/calcs_in_repeats/rep1/num1"]:eq(2)')
            .val('40')
            .trigger('change');
        ...
        expect(
            form.model.node('/calcs_in_repeats/rep1/grp/num1', 2).getVal()
        ).to.equal('800');
    });
    ...
    it('inside a repeat using the position(..) function are updated if the position changes due to repeat removal', () => {
        ...
        expect(form.model.xml.querySelectorAll('pos')[1].textContent).to.equal(
            '2'
        );
        expect(
            form.view.$.find('.or-output[data-value="/RepeatGroupTest/P/pos"]')
                .eq(1)
                .text()
        ).to.equal('2');
        expect(form.model.xml.querySelectorAll('pos')[2].textContent).to.equal(
            '3'
        );
        expect(
            form.view.$.find('.or-output[data-value="/RepeatGroupTest/P/pos"]')
    ...
    it('do not cause an exception when a repeat is removed', () => {
        ...
        addButton.click();
        const deleteAction = () => {
            [...form.view.html.querySelectorAll('.remove')].pop().click();
        };
...
describe('branching functionality', () => {
    ...
    it('hides non-relevant branches upon initialization', () => {
        ...
        form.init();
        expect(
            form.view.$.find('[name="/data/group"]').hasClass('disabled')
        ).to.equal(true);
        expect(
            form.view.$.find('[name="/data/nodeC"]').parents('.disabled').length
        ).to.equal(1);
    });
    ...
    it('a) evaluates relevant logic on a repeated radio-button-question and b) injects the position correctly (issue 208)', () => {
        ...
        expect(form.model.node('/issue208/rep/nodeA', 1).getVal()).to.equal(
            'yes'
        );
    ...
    it('re-evaluates when a node with a relative path inside a relevant expression is changed', () => {
        ...
        const $a = $form.find('[name="/relative/a"]');
        const $branch = $form
            .find('[name="/relative/c"]')
            .closest('.or-branch');
    ...
    describe('inside repeats when multiple repeats are present upon loading', () => {
        it('correctly evaluates the relevant logic of each question inside all repeats (issue #507)', () => {
            ...
            form.init();
            const $relNodes = form.view.$.find(
                '[name="/multiple_repeats_relevant/rep/skipq"]'
            ).parent('.or-branch');
        ...
        it('correctly evaluates the relevant logic of each simple select question inside all repeats (issue #442 core)', () => {
            const form = loadForm(
                'repeat-relevant-select1.xml',
                '<Enketo_tests><details><fruits>pear</fruits><location></location></details><details><fruits>mango</fruits><location>kisumu</location></details><details><fruits>mango</fruits><location>kisumu</location></details><meta><instanceID>a</instanceID></meta></Enketo_tests>'
            );
            form.init();
            const $relNodes = form.view.$.find(
                '[data-name="/Enketo_tests/details/location"]'
            ).closest('.or-branch');
    ...
    describe('in nested repeats with a <select> that has a relevant', () => {
        ...
        const instanceStr =
            '<data><region><livestock><type>d</type><type_other/></livestock></region><region><livestock><type>d</type></livestock></region><meta><instanceID>a</instanceID></meta></data>';
    ...
    describe('handles clearing of form control values in non-relevant branches', () => {
        ...
        it('by not clearing UPON LOAD', () => {
            ...
            form.init();
            expect(
                form.view.$.find(`[name="${two}"]`).closest('.disabled').length
            ).to.equal(1);
            expect(
                form.view.$.find(`[name="${three}"]`).closest('.disabled')
                    .length
            ).to.equal(1);
    ...
    describe('handles calculated values in non-relevant/relevant branches with default settings', () => {
        ...
        const groupReadonlyCalc =
            '/calc-in-group-with-relevant/grp/groupReadonlyCalc';
    ...
    describe('on a group with an expression that refers to a repeat node value', () => {
        it('re-evaluates when the referred node changes', () => {
            ...
            form.init();
            expect(
                form.view.$.find('[name="/data/LYMPHNODES/LYMNDISS"]').closest(
                    '.disabled'
                ).length
            ).to.equal(1);
            form.view.$.find('[name="/data/PROCEDURE/PROC_GRID/PROC"]')
                .val('6')
                .trigger('change');
            expect(
                form.view.$.find('[name="/data/LYMPHNODES/LYMNDISS"]').closest(
...
describe('obtaining XML string from form without irrelevant nodes', () => {
    ...
    it('works for a nested branch where there is an irrelevant descendant of an irrelevant ancestor', () => {
        ...
        const match = '<c/>';
        form.init();
        form.view.$.find('[name="/nested-branches/b"]')
            .val(0)
            .trigger('change');
    ...
    it('works for nested repeats where some children are irrelevant', () => {
        ...
        const instanceStr =
            '<data><region><livestock><type>d</type><type_other/></livestock><livestock><type>other</type><type_other>one</type_other></livestock></region><region><livestock><type>d</type><type_other/></livestock><livestock><type>other</type><type_other>two</type_other></livestock></region><meta><instanceID>a</instanceID></meta></data>';
        const form = loadForm('nested-repeat-v5.xml', instanceStr);
        form.init();
        ...
        expect(
            ...
            '<region><livestock><type>d</type><type_other/></livestock><livestock><type>other</type><type_other>one</type_other></livestock></region><region><livestock><type>d</type><type_other/></livestock><livestock><type>other</type><type_other>two</type_other></livestock></region>'
        );
        ...
        expect(
            ...
            '<region><livestock><type>d</type></livestock><livestock><type>other</type><type_other>one</type_other></livestock></region><region><livestock><type>d</type></livestock><livestock><type>other</type><type_other>two</type_other></livestock></region>'
        );
    });
    ...
    it('works for simple "select" (checkbox) questions inside repeats', () => {
        ...
        form.init();
        form.view.$.find('.add-repeat-btn').click().click();
        form.view.$.find(repeat)
            .eq(0)
            .find(`${fruit}[value="pear"]`)
            .prop('checked', true)
            .trigger('change');
        form.view.$.find(repeat)
            .eq(1)
            .find(`${fruit}[value="mango"]`)
            .prop('checked', true)
            .trigger('change');
        form.view.$.find(repeat)
            .eq(2)
        ...
        expect(
            ...
            '<details><fruits>pear</fruits></details><details><fruits>mango</fruits><location>nairobi</location></details><details><fruits>pear</fruits></details>'
        );
    });
});
...
describe('Required questions', () => {
    ...
    it('dynamically update the asterisk visibility in real-time', () => {
        ...
        const $asterisk = form.view.$.find('[name="/required/b"]')
            .closest('.question')
            .find('.required');
...
describe('jr:choice-name', () => {
    ...
    it('should match when there are spaces in arg strings', () => {
        ...
        expect(
            form.view.$.find('[name="/choice-regex/translator"]:checked')
                .next()
                .text()
        ).to.equal('[Default Value] Area');
        expect(
            form.view.html.querySelectorAll('.readonly .or-output')[0]
                .textContent
        ).to.equal('[Default Value] Area');

        // when
        form.view.$.find('[name="/choice-regex/input"]')
            .val('abc')
            .trigger('change');
        ...
        // when
        form.view.$.find(
            '[name="/choice-regex/translator"][value=health_center]'
        )
            .click()
            .trigger('change');
    ...
    it('should match when there the path parameters are relative', () => {
        ...
        expect(
            form.view.html.querySelectorAll('.readonly .or-output')[1]
                .textContent
        ).to.equal('[Default Value] Area');
    });
    ...
    it('should handle regression reported in issue #490', () => {
        ...
        // then
        expect(form.view.$.find('.readonly .or-output').text()).to.equal(
            'unspecified'
        );

        // when
        form.view.$.find('[name="/embedded-choice/translator"][value=clinic]')
            .click()
            .trigger('change');
    ...
    it('should work with autocomplete select one questions', () => {
        ...
        form.init();
        form.view.$.find('[name="/Label.Test/choice"]')
            .val('3')
            .trigger('change'); // Doesn't update view, but changes model.

        expect(form.view.$.find('.or-output').text()).to.equal(
            'The Third Choice'
        );
    });
    ...
    it('should work with radio buttons', () => {
        ...
        expect(
            form.view.html.querySelector('[data-name="/data/r1/province_name"]')
                .checked
        ).to.equal(false);

        form.view.html.querySelector(
            '[data-name="/data/r1/province_name"]'
        ).checked = true;
        form.view.html
            .querySelector('[data-name="/data/r1/province_name"]')
            .dispatchEvent(events.Change());

        expect(
            form.view.html.querySelector('[data-value=" ../province_label "]')
                .innerText
        ).to.equal('Central');
    });
    ...
    it('should work with **empty** lists in pulldown selects', () => {
        ...
        expect(
            form.model.xml.querySelector('questionname').textContent
        ).to.equal('A');
    });
});
...
describe('autocomplete questions', () => {
    ...
    it('populate correctly if a single language is defined in the form, and the Form instance is instantiated using that language explicitly', () => {
        const form = loadForm('autocomplete-cascade.xml', null, {
            language: 'en',
        });
        form.init();
        const datalists = form.view.html.querySelectorAll('datalist');
        const firstDatalistContent = [
            ...datalists[0].querySelectorAll('option'),
        ].map((el) => `${el.value}:${el.dataset.value}`);
        expect(firstDatalistContent).to.deep.equal([
            '...:',
            'A:a',
            'B:b',
            'C:c',
        ...
        const secondDatalistContent = [
            ...datalists[1].querySelectorAll('option'),
        ].map((el) => `${el.value}:${el.dataset.value}`);
        expect(secondDatalistContent).to.deep.equal([
            ':undefined',
            'BA:ba',
            'BB:bb',
            'BC:bc',
...
describe('Form.prototype', () => {
    ...
    describe('#replaceChoiceNameFn()', () => {
        const tests = {
            'jr:choice-name( /choice-regex/translator, " /choice-regex/translator ")':
                '"__MOCK_VIEW_VALUE__"',
            '     jr:choice-name(       /choice-regex/translator     ,  " /choice-regex/translator "   )    ':
                '     "__MOCK_VIEW_VALUE__"    ',
            "if(string-length( /embedded-choice/translator ) !=0, jr:choice-name( /embedded-choice/translator ,' /embedded-choice/translator '),'unspecified')":
                'if(string-length( /embedded-choice/translator ) !=0, "__MOCK_VIEW_VALUE__",\'unspecified\')',
            'jr:choice-name( selected-at( /energy_1/light/light_equip , 1), " /energy_1/light/light_equip " )':
                '"__MOCK_VIEW_VALUE__"',
            'if( /data/C1 =01, jr:choice-name( /data/C2 ," /data/C2 "), jr:choice-name( /data/C3 ," /data/C3 " ) )':
        ...
        for (const [initial, expected] of Object.entries(tests)) {
            it(`should replace ${initial} with ${expected}`, () => {
                ...
                // when
                const actual = Form.prototype.replaceChoiceNameFn.call(
                    form,
                    initial
                );
